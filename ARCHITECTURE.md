...
- **Per-tenant fair-share sub-limiter**: each tier's global semaphore is preceded by a per-tenant channel sized `MAX_SCRAPE_CONCURRENCY_PER_TENANT` (default: the global cap itself), keyed by `auth.TenantIDFromContext(ctx)` and never a tool-input field. A tenant acquires its own slot first, then the global one — a tenant hitting its own cap queues without touching other tenants' capacity, and total in-flight scrapes across all tenants still never exceeds the global ceiling. The per-tenant limiter map lives on the `Pipeline` instance (no package-level state) and is bounded/LRU-evicted the same way `internal/metrics/collector.go`'s `tenantStats` map is (#475), so a client cycling through many distinct tenant IDs can't grow it unboundedly (#463)
