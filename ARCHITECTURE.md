...
- **`FilingSearcher` / `CaseSearcher` / `EconSearcher` / `TrialSearcher`** — `internal/search/structured_domains.go`. The structured-research domains behind `filing_search` (SEC EDGAR), `legal_search` (CourtListener), `econ_search` (FRED + World Bank), and `clinical_search` (ClinicalTrials.gov). Each follows the same `…Searcher` + `…Provider` + `Supported…Providers` + `New…ByName` + `Available…Providers` shape as the patent/academic capabilities, resolved from the `Dependencies` maps in the tool layer. A new provider behind an existing interface (e.g. World Bank under `EconProvider`) is one factory case + one list entry, no tool change.
