...
The five boxes above are a schematic, not an exhaustive tool list — the actual tool count has grown well past what fits in five boxes. For the full, current tool inventory grouped by category, see `internal/tools/registry.go`'s `RegisterAll()` (the source of truth) or `docs/TOOLS.md`. As of this writing that includes, beyond the five shown: structured-domain research (`filing_search`, `legal_search`, `econ_search`, `clinical_search`, `awesome_list_search`, `monarch_search`, `syllabus_search`, `gag_order_search`), OSINT/company reconnaissance (`company_recon`), single-call paper retrieval (`paper_fulltext`), synthesis (`answer`, `structured_search`), place/context search (`local_search`), citation tooling (`citation_graph`, `verify_citation`, `audit_bibliography`, `format_bibliography`, `research_export`), trust/anti-sloptimization (`verify_recommendation`, `archive_source`, `brand_research`), and the opt-in regulated tools (`memory_save`/`memory_recall`, `workspace_contribute`/`workspace_read`, `get_my_analytics`). `syllabus_search` and `gag_order_search` register conditionally (`OPEN_SYLLABUS_API_KEY`+`OPEN_SYLLABUS_API_URL`, and `PEN_AMERICA_AIRTABLE_TOKEN`, respectively); the rest in this sentence are always registered (keyless backing data sources).
