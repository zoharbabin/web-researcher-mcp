...
1. **Developer ID Application certificate (`.p12`):** at developer.apple.com → Certificates, create a CSR (Keychain Access) → certificate type **Developer ID Application** (not "Installer", not "Apple Development") → download `.cer` → import to Keychain → export with private key as `.p12` with a password.
   - **The `.p12` MUST carry the full Apple chain** — the leaf *and* the "Developer ID Certification Authority" intermediate *and* the "Apple Root CA". A leaf-only `.p12` makes `quill` emit an **unsatisfiable designated requirement** (`certificate root[...]` instead of `certificate 1[...]`), and AMFI then SIGKILLs the launched binary (issue #204). Build the full-chain `.p12` from the raw key + certs (the Apple Root CA is public, from <https://www.apple.com/certificateauthority/>): `openssl pkcs12 -export -inkey devid_app.key -in leaf.pem -certfile <(cat DeveloperIDG2CA.pem AppleRootCA.pem) -legacy -passout pass:<pwd> -out devid_app.p12`. Verify with `quill p12 describe devid_app.p12` — it must list a "Certificate Chain: (2)" of the intermediate + root.
   - `base64 -i devid_app.p12 | pbcopy` → `gh secret set MACOS_SIGN_P12` (paste); password → `gh secret set MACOS_SIGN_PASSWORD`.
   - The release workflow's **"Verify macOS signing chain"** step re-checks all three certs are present before GoReleaser signs, and fails the release fast if a leaf-only `.p12` is ever re-uploaded.
