...
- SHA-256 checksums
- A SLSA build provenance attestation covering every checksummed artifact, generated by a workflow isolated from the build job (see below)
...
# 4. Verify the SLSA build provenance attestation for a downloaded artifact
#    (binary, archive, or checksums.txt) using the GitHub CLI — no cosign
#    needed for this check, `gh` talks to the GitHub attestations API directly.
gh attestation verify web-researcher-mcp_1.48.0_linux_amd64.tar.gz \
  --owner zoharbabin
```

Artifact attestations are generated by a dedicated reusable workflow (`.github/workflows/provenance.yml`), isolated from the GoReleaser build job so that nothing running inside the build (including third-party GoReleaser plugins) can influence what gets attested — this isolation, combined with GitHub's ephemeral hosted runners and Sigstore keyless signing, satisfies SLSA Build Level 3's provenance requirements.
...
| `actions/attest-build-provenance` | SLSA build provenance attestation | Every release |
