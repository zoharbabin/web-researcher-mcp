...
- **RIS** records use `TY  - JOUR` when a DOI is present (the entry is almost certainly a journal article); others use `TY  - ELEC`. One `AU` line per author — the `author` string is split on `;`, ` and `, or a bare comma between two or more full names (e.g. "Yoshua Bengio, Aaron Courville", #639) — with `DO` carrying the bare DOI and `UR` the URL; values are stripped of line breaks so a title can't inject extra RIS tags.
...
- A multi-author `author` string with no `;`/` and ` separator still splits correctly in two shapes: a bare comma-separated list of full names (each comma-delimited part has 2+ words, e.g. "Yoshua Bengio, Aaron Courville", #639), or a bare comma-separated list of pre-inverted pairs (every part is a single token, even count, e.g. "Smith, John, Doe, Jane" → "Smith, John" and "Doe, Jane", #680). A single pre-inverted "Family, Given" name (e.g. "Bengio, Yoshua" or "Smith, J.") is left alone, and a mixed list (odd count, or a mix of single- and multi-word parts) is left unsplit as genuinely ambiguous.
...
- **Claim check (optional).** When a `claim` is given, the source is fetched (the live URL, a matched record's URL, or a Wayback snapshot) and measured for topical overlap with the same lexical, model-free coverage as `audit_bibliography` (no model/embedding): `claimSupport` reports COVERAGE not stance, `not_addressed` is the mischaracterization signal (only when a source was actually read), and `contrastSignal` flags a negation cue. For a DOI/reference match, the fetch tries a priority-ordered list of candidate URLs rather than committing to one: Unpaywall's independently-queried `best_oa_location` (#657 — a second, freshly-resolved OA signal, since the record's own cached pick can go stale), then the matched record's own open-access/PDF URL, then a non-redirect landing page, then a `doi.org` redirect — retrying past a candidate that comes back `source_unavailable` (a fetch error, or a fetch that succeeded but returned no usable content, e.g. a bot-wall serving an empty 200) until one succeeds or 4 attempts are exhausted (#681). `source_unavailable` when no fetchable candidate exists at all (e.g. a DOI/reference whose matched record carries no URL and Unpaywall has no OA copy). Off — and zero added latency — unless a claim is supplied. `claimEvidence` and `contrastSignal` are English-keyword heuristics (#390): on non-English source text an empty/false result means the heuristic didn't match, not that the claim is confirmed unaddressed/unopposed — read the source yourself when it isn't English. The same caveat applies to `conflictOfInterest`, which matches English employment/funding/equity phrases against author bio text.
...
- Returns an empty `places` array (with `hints`) when no location results match; never panics. When zero results come back and neither `near` nor `latitude`/`longitude` was set, `hints.reason` is `missing_location_anchor` with a leading `suggestedActions` entry naming `near` — a location stated only in free-text `query` (e.g. `"coffee shop near Times Square New York"`) is invisible to the provider, since local intent is anchored exclusively via `near`/coordinates, never by appending location text to the query (#634). When an anchor was set and still returns zero results, `hints.reason` falls back to the shared `no_match`/`filters_too_restrictive` behavior.
...
- **Claim check (optional, #174).** When an entry carries a `claim`, the source page is fetched (the live URL, or its Wayback snapshot when the live link is dead) and measured for topical overlap via transparent term coverage. `claimSupport` reports **coverage, not a stance**: `addressed` (strong overlap — claim-relevant sentences returned in `claimEvidence` for you to judge direction), `partially_addressed` (some overlap — evidence shown but **not** flagged; ambiguous, you judge), `not_addressed` (the source addresses **none** of the claim → the `mischaracterized` flag), or `source_unavailable` (no fetchable source). It never asserts "supports"/"refutes" — the extractor surfaces sentences, not direction — and the flag fires only on zero overlap of a fetched source, so a real-but-tangential source is never falsely accused (under-flagging is the safe direction). The check is **lexical, not semantic** (no model/embedding dependency): coverage is measured as the **peak overlap within a sentence window** (not across the whole page), so a narrow claim whose terms are merely scattered across a long, broad article scores low local coverage rather than a misleading `partially_addressed`. `addressed` additionally requires that if the claim has a distinguishing (proper-noun-like) term — one capitalized mid-claim rather than merely at the start of a sentence, e.g. "Alzheimer's" in a claim otherwise made of generic/topical vocabulary — at least one such term actually appears in that SAME peak coverage window (not merely anywhere else on the page, which would let a stray unrelated mention satisfy the check); otherwise the result is capped at `partially_addressed`, since generic-term overlap alone can clear the ratio even when the one term that would make the claim true (or false) never matched locally (#675). A claim made entirely of generic vocabulary has no such term and is unaffected — pure ratio coverage as above. Borderline partial overlap is still shown as evidence for you to judge, not flagged. Because a source can share a claim's terms while *contradicting* it, a `contrastSignal: true` is added when a claim-relevant sentence carries a negation/refutation cue (e.g. "no significant", "did not", "no evidence", "contradicts") — a neutral "read this sentence yourself" heads-up so an `addressed` result is never mistaken for confirmation. The cue list holds only explicit negation/refutation terms, never bare discourse connectives like "however"/"although"/"whereas" (which contrast two things without opposing the claim and would false-positive on supporting sources). Off unless a claim is given (no added latency otherwise). `claimEvidence` and `contrastSignal` are English-keyword heuristics (#390): on non-English source text an empty/false result means the heuristic didn't match, not that the claim is confirmed unaddressed/unopposed — read the source yourself when it isn't English.
...
`identifier` (echo), `resolvedUrl` (the URL actually scraped — the open-access PDF, the Semantic Scholar landing page, the doi.org redirect, or the input URL verbatim), `content`, `title`, `trust` (`untrusted-external-content`), `truncated`, `scrapeTier` (which extraction tier produced the content, when known), `source` (`semanticscholar` when a DOI/paper-ID lookup succeeded, `unpaywall` when neither Semantic Scholar nor OpenAlex had a record but Unpaywall resolved a PDF directly, `openalex` when Semantic Scholar had no record for the DOI but an exact-DOI OpenAlex lookup did, `direct-url` for a URL input or when no metadata could be resolved), `metadataIncomplete` (`true` when title, author, and publish date are all empty — the `citation` field is a placeholder, not a real citation; omitted otherwise), `citation` (the same `url`/`accessedDate`/`metadata`/`formatted` shape as `scrape_page`'s citation object). When `source` is `semanticscholar`, `unpaywall`, or `openalex`: `doi`, `pdfUrl`, `openAccess` (plus, when known: `authors`, `year`, `citationCount`, `abstract`, `journal`, `tldr` — AI-generated one-sentence summary, attribute as AI-generated).
...
- **OA candidate gathering and liveness verification (#657).** For a DOI, every known open-access signal is gathered into one priority-ordered candidate list — the fetcher's own cached PDF URL, Unpaywall's live `best_oa_location` lookup, then an exact-DOI OpenAlex lookup's cached OA URL — instead of trusting a single provider's cached pick. When a link verifier is configured (`deps.LinkVerifier`), each candidate is checked for liveness in priority order and the first live one wins; a dead or bot-walled top-ranked candidate falls through to the next rather than being returned unconditionally. With no link verifier configured, the top-ranked candidate is used as-is (unchanged behavior).
- **OpenAlex metadata fallback (#658).** When Semantic Scholar has no record at all for the DOI (not configured, no record, or an upstream fetch error), an exact-DOI OpenAlex lookup (the `DOIResolver` capability) is attempted for bibliographic metadata — title, authors, year — not just a PDF URL, so a resolvable DOI doesn't degrade to a titleless, authorless citation just because Semantic Scholar has no record of it. A hit returns `source:"openalex"`.
- **URL resolution order** for a resolved paper, after the above: the winning OA candidate; then the Semantic Scholar/DOI/arXiv landing page URL; then (DOI input only) the `https://doi.org/{doi}` redirect.
- **`metadataIncomplete` flag.** When no metadata source (Semantic Scholar, OpenAlex, the scraped page's own `<title>`/author/date metadata) yields anything at all, the response still succeeds but sets `metadataIncomplete:true` rather than silently emitting a placeholder `(n.d.).`-style citation that looks complete.
