...
- A multi-author `author` string with no `;`/` and ` separator still splits correctly when it's a bare comma-separated list of full names (each comma-delimited part has 2+ words, e.g. "Yoshua Bengio, Aaron Courville"). A single pre-inverted "Family, Given" name (e.g. "Bengio, Yoshua" or "Smith, J.") is left alone — the split only fires when every comma-delimited part looks like a full name, not a bare surname or initial (#639).
