...
- **Claim check (optional).** When a `claim` is given, the source is fetched (the live URL, a matched record's URL, or a Wayback snapshot) and measured for topical overlap with the same lexical, model-free coverage as `audit_bibliography` (no model/embedding): `claimSupport` reports COVERAGE not stance, `not_addressed` is the mischaracterization signal (only when a source was actually read), and `contrastSignal` flags a negation cue. For a DOI/reference match, the fetch tries a priority-ordered list of candidate URLs rather than committing to one: the matched record's own open-access/PDF URL, Unpaywall's independently-queried `best_oa_location` (#657 — a second, freshly-resolved OA signal, since the record's own cached pick can go stale), then a non-redirect landing page, then a `doi.org` redirect — retrying past a candidate that comes back `source_unavailable` (a fetch error, or a fetch that succeeded but returned no usable content, e.g. a bot-wall serving an empty 200) until one succeeds or 3 attempts are exhausted (#681). `source_unavailable` when no fetchable candidate exists at all (e.g. a DOI/reference whose matched record carries no URL and Unpaywall has no OA copy). Off — and zero added latency — unless a claim is supplied. `claimEvidence` and `contrastSignal` are English-keyword heuristics (#390): on non-English source text an empty/false result means the heuristic didn't match, not that the claim is confirmed unaddressed/unopposed — read the source yourself when it isn't English. The same caveat applies to `conflictOfInterest`, which matches English employment/funding/equity phrases against author bio text.
