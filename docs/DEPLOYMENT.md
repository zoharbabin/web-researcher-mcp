...
|----------|-------------|---------|
| `SEARCH_PROVIDER` | Primary provider: google, brave, serper, searxng, searchapi, youcom, duckduckgo, tavily, exa, hackernews, reddit, bluesky, github, xquik | `google` (variable default); at runtime, when `google` is selected but no Google key is set, the server falls back to the zero-config `duckduckgo` provider |
...
| `GITHUB_TOKEN` | `awesome_list_search`, `web_search`/`news_search` (`provider=github`), `scrape_page` | Optional token raising GitHub's public REST API rate limits (Search API, Contents/Gists API, and the trust-surface calls below). Every surface it touches **is always available** unauthenticated at GitHub's documented public rate limit; the token only raises that ceiling, never gates functionality. No `gh` CLI, no subprocess. Never logged. Create a scopeless fine-grained PAT at github.com/settings/tokens. `scrape_page` on a github.com repo-root README adds 5 REST calls (repo stats, owner/org profile, contributor count, community health, release count) against that same core rate limit (60/hr unauthenticated, 5000/hr with the token) — a 403/rate-limited response on any one of them is never retried and just omits that field (graceful degradation), never the whole scrape | — |
...
- **Workspaces:** `internal/workspace`'s `Store` is built from the same `persistStore` as sessions and rate-limit quota (`cmd/web-researcher-mcp/main.go`), so shared workspaces are per-instance too — a member reading from a different pod than the one that last wrote won't see the update. Same per-pod exposure as sessions/rate limits, same fix.
...
- [ ] **Shared workspaces across pods** — without Redis, `WORKSPACES_ENABLED` workspace state (`internal/workspace`) lives in the same per-pod `persistStore` as sessions, so a member on a different pod than the last writer won't see recent contributions. Use `REDIS_URL` for fleet-wide workspace visibility, or sticky sessions as a stopgap. **(per-pod without Redis)**
