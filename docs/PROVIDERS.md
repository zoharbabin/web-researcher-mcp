...
| `awesome_list_search` | **[ecosyste.ms](https://ecosyste.ms/)** | Community-curated "awesome list" discovery by topic — stars, project counts, archived status | No (`ECOSYSTEMS_API_KEY` optional, raises rate-limit tier) |
...
**Notes:**
- World Bank, OECD, Eurostat, ClinicalTrials.gov, CourtListener, and ecosyste.ms are always available — no configuration required. Setting `ECOSYSTEMS_API_KEY` raises ecosyste.ms's per-caller rate-limit tier above the shared "anonymous" pool.
