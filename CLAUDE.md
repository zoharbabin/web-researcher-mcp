...
4. To also offer a specialized capability (academic / answer / structured / citation / filing / case / econ), implement the matching `…Provider` interface and register the name in its `Supported…Providers` list + `New…ProviderByName` switch + `Available…Providers` constructor: `internal/search/domain.go` for academic + citation, `synthesis.go` for answer/structured, `structured_domains.go` for the filing/case/econ trio (EDGAR/CourtListener/FRED). The matching tool (`academic_search`/`answer`/`structured_search`/`citation_graph`/`filing_search`/`legal_search`/`econ_search`) then picks it up with no tool-layer change. Every provider gets its own `circuit.New(...)` breaker via the `Available…Providers` constructor.
