...
- **GitHub issues are specs, not scratchpads.** An issue body states current codebase state and what to build — concise, concrete, ready to implement. Never post a comment narrating your own investigation, hedges, or "here's what I checked" reasoning — that's internal process, not spec content, and it wastes tokens for every agent that reads the issue afterward. When an issue's cited state goes stale (a blocker ships, a file moves), edit the body in place to reflect current reality — don't append a comment layering old-vs-new. Comments are for genuine back-and-forth with external contributors only.
...
8. **Provider maps** — `deps.SearchProviders`, `deps.PatentProviders`, `deps.AcademicProviders`, `deps.FilingProviders`, `deps.CaseProviders`, `deps.EconProviders`, `deps.TrialProviders`, `deps.AwesomeListProviders`, `deps.LocalProviders`, `deps.ContextProviders`, `deps.MonarchProviders` hold all configured providers by name; built at startup via `Available…Providers()`, independent of routing config
...
2. Add `register<Name>(srv, deps)` to `RegisterAll()` in `internal/tools/registry.go`. **Regulated/opt-in tools** register conditionally (only when their feature dep is non-Noop) and gate every personal-data operation on `deps.Consent.HasConsent(ctx, consent.PurposeMemory)` (single purpose per call site); register the store's `Exporter`/`Eraser` into the `datasubject` registry in `main.go`. Mirror this pattern for any new tool with a required credential (email, API key, or token) — not `legal_search`, which registers unconditionally. See `filing_search` (`EDGAR_CONTACT_EMAIL`/`OPENALEX_EMAIL`), `syllabus_search` (`OPEN_SYLLABUS_API_KEY`/`OPEN_SYLLABUS_API_URL`), and `gag_order_search` (`PEN_AMERICA_AIRTABLE_TOKEN`) for examples of email- and token-gated tools.
...
4. Document the schema in `docs/TOOLS.md` as a `## Tool N: \`name\`` section (the drift test parses these headers). **When modifying an existing tool** (renaming params, changing descriptions, adding/removing fields), update the matching `docs/TOOLS.md` section in the same commit — `TestToolsDocMatchesRegistry` fails CI on any drift, including edits to existing tools. **If the tool is a structured-domain capability** (its own dedicated provider, like `filing_search`/`legal_search`/`econ_search`/`clinical_search`/`awesome_list_search`, or a single-purpose tool like `archive_source`/`brand_research`), also add its row to `docs/PROVIDERS.md`'s "Structured-Domain Providers" table and to `structuredDomainDocTools` in `metadata_test.go` — `TestProvidersDocStructuredDomainTable` fails CI on any drift there.
...
4. For specialized capabilities (academic / citation / filing / case / econ / trial): implement the matching `…Provider` interface and register in its `Supported…Providers` list + `New…ProviderByName` switch + `Available…Providers` constructor — `internal/search/domain.go` (academic + citation), `structured_domains.go` (filing/case/econ/trial). The matching tool picks it up with no tool-layer change. Every provider gets its own `circuit.New(...)` breaker via the `Available…Providers` constructor.
...
- `internal/tools/metadata_test.go` fails CI on drift: `TestToolsDocMatchesRegistry` (docs/TOOLS.md ↔ registry), `TestProvidersDocStructuredDomainTable` (docs/PROVIDERS.md's Structured-Domain table ↔ registry), `TestAllToolsHaveAnnotations`, `TestOutputSchemaMatchesResponse`, `TestToolDescriptionQuality`. These also run in a standalone `docs-drift` CI job on docs-only PRs.
- `ARCHITECTURE.md`, `docs/EXTENSION_GUIDE.md`, and `docs/EXAMPLES.md` are prose/illustrative references with no CI drift gate — keep them current by hand when adding a tool or capability interface; there is no mechanical check to catch omissions here.
