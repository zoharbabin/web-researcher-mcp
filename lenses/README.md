...
A lens **must** define at least one of `domains` or `cx` — otherwise it never restricts a search. This is enforced by `search.ValidateLens` (`internal/search/lenses.go`); an invalid lens fails `make validate-lenses` and, in HTTP mode, fails startup.

## Add a bundled lens

1. Add `lenses/<name>.json` following the schema above (template: [`academic.json`](academic.json)).
2. `make validate-lenses` to check it.
3. It auto-loads — no code change.
...
| `finance` | `filing_search` (SEC EDGAR) + `econ_search` (FRED) | Primary financial disclosures and economic series. |
