...
| `awesome-lists` | `scrape_page` + `verify_recommendation` | Community-curated, PR-reviewed `awesome-*` tool and resource lists on GitHub and their aggregator mirrors (`project-awesome.org`, `awesomeopensource.com`, `awesome.ecosyste.ms`, etc.) — a vendor-neutral shortlist generator, not a maintenance-status oracle. Pair with `verify_recommendation` (or `lens: "security"`) to check a shortlisted tool is still actively maintained before treating it as an actionable recommendation. |
| `curriculum` | `syllabus_search` (Open Syllabus) + `gag_order_search` (PEN America) | Academic curriculum data (`opensyllabus.org`, `eric.ed.gov`), country-level academic freedom indices (`academic-freedom-index.net`, `v-dem.net`, `scholarsatrisk.org`, `freedomhouse.org`), US institutional climate (`fire.org`, `aaup.org`, `heterodoxacademy.org`, `knightfoundation.org`, `pen.org`), US campus incident tracking across the political spectrum (`adl.org`, `amchainitiative.org`, `hillel.org`, `campusreform.org`, `nas.org`, `palestinelegal.org`), international climate (`cst.org.uk`, `freespeechunion.org`, `eua.eu`, `europarl.europa.eu`), and global human-rights/statistics (`hrw.org`, `amnesty.org`, `ohchr.org`, `uis.unesco.org`, `oecd.org`, `timeshighereducation.com`). `campusreform.org` requires the browser scrape tier (bot protection); `hrw.org`/`amnesty.org`/`oecd.org`/`timeshighereducation.com` are large multi-topic sites — use section-specific URLs with `scrape_page` (e.g. `hrw.org/topic/academic-freedom`). Excludes `caut.ca` (dead URLs), `ujs.org.uk` (TLS cert mismatch), and `canarymission.org` (individual-profiling watchlist, active class-action litigation) — not curriculum data. |
