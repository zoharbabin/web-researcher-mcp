...
func registerGetSession(srv *mcp.Server, deps Dependencies) {
	mcp.AddTool(srv, &mcp.Tool{
		...
		Description:  "Recover a sequential_search research session after context loss. Returns the session summary, a one-liner step index covering every step, and the last 3 steps in full detail (the `lastSteps` sliding window). This overview's `responseMode` is always \"summary\" — it is not governed by any step-count threshold, unlike sequential_search's own `responseMode` field (auto: full for 8 or fewer steps). For full details of any earlier step, pass its stepId, which returns responseMode \"step\". A source's `foundInStep` is the 1-indexed step that surfaced it, omitted when the source was not tied to a numbered step (e.g. added via a web_search carrying only a sessionId) — there is no step 0. Sessions persist for 4 hours from last activity and survive server restarts.",
