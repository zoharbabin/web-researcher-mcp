...
// back to the raw source data so nothing is hidden or unverifiable. Components
// are NEVER a substitute for the raw text/sources, which are always present.

// AutoFormattedLabel is the non-disableable marker stamped on every generated
// component. It is deliberately "mcp-auto-formatted", NOT "AI-formatted": these
// components are assembled by the MCP server via a deterministic transform of
// already-extracted data — there is no model involved. The label tells clients
// the MCP shaped this structure (not another component, and not an LLM).
const AutoFormattedLabel = "mcp-auto-formatted"
...
type Component struct {
	...
	AutoFormatted bool            `json:"autoFormatted"` // always true; the mandatory label
	Label         string          `json:"label"`         // AutoFormattedLabel
...
func autoComponent(typ, title string, refs []string) Component {
	return Component{
		Type:          typ,
		AutoFormatted: true,
		Label:         AutoFormattedLabel,
		Title:         title,
		SourceRefs:    refs,
	}
}

// BuildComponents assembles mcp-auto-formatted components from scored sources. It is
...
func BuildComponents(sources []ScoredSource, snippets map[string]string) []Component {
	...
	for _, s := range withText {
		card := autoComponent("card", s.Title, []string{s.URL})
	...
	if len(withText) >= 2 {
		...
		table := autoComponent("table", "Source quality comparison", refs)
