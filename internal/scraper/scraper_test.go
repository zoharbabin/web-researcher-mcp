...
func TestParseTranscriptVTT_AutoGeneratedCTags(t *testing.T) {
	// Real YouTube auto-generated (kind=asr) captions interleave word-role <c>
	// tags with inline timing tags: both must be stripped, not just the timing.
	vtt := "WEBVTT\nKind: captions\nLanguage: en\n\n" +
		"00:00:00.000 --> 00:00:00.560\nhe<00:00:00.320><c> is</c>\n\n" +
		"00:00:00.560 --> 00:00:01.200\nhe<00:00:00.320><c> is</c><00:00:00.560><c> a</c>\n"
	result := parseTranscriptVTT(vtt)
	if strings.Contains(result, "<c>") || strings.Contains(result, "</c>") {
		t.Errorf("expected <c> role tags stripped, got %q", result)
	}
	if !strings.Contains(result, "[0:00] he is") {
		t.Errorf("expected clean text 'he is', got %q", result)
	}
}
